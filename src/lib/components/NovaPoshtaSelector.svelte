<script lang="ts">
	...
	function handleOutsideClick(event: MouseEvent) {
		if (cityInputRef && cityDropdownRef && showCityDropdown) {
			if (
				!cityInputRef.contains(event.target as Node) &&
				!cityDropdownRef.contains(event.target as Node)
			) {
				showCityDropdown = false;
			}
		}
		...
		if (warehouseInputRef && warehouseDropdownRef && showWarehouseDropdown) {
			if (
				!warehouseInputRef.contains(event.target as Node) &&
				!warehouseDropdownRef.contains(event.target as Node)
			) {
				showWarehouseDropdown = false;
			}
		}
	...
	const filteredWarehouses = $derived(
		warehouseSearchQuery
			? $warehouses.filter((w) =>
					w.Description.toLowerCase().includes(warehouseSearchQuery.toLowerCase())
				)
			: $warehouses
	);
...
{#if $pageTranslations}
	<div class="nova-poshta-selector">
		<div class="np-section">
			<label for="np-city-input" class="np-label"
				>{$pageTranslations.t('delivery.npCity') as string}</label
			>
		...
		{#if selectedCityName}
			<div class="np-section">
				<label for="np-warehouse-input" class="np-label"
					>{$pageTranslations.t('delivery.npWarehouse') as string}</label
				>
