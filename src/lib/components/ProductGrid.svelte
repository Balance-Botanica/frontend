...
{#if enablePagination && totalPages > 1}
	<div class="mt-8 flex items-center justify-center gap-2">
		<button 
			class="touch-button px-4 py-2 rounded-lg border border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50"
			on:click={prevPage}
		...
		{#each Array(totalPages) as _, i}
			<button
				class={`touch-button px-4 py-2 rounded-lg ${i + 1 === currentPage ? 'bg-green-600 text-white' : 'border border-gray-300 hover:bg-gray-50'}`}
