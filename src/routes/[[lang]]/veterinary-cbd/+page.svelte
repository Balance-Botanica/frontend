<script lang="ts">
	...
	const translations = $derived(() => ({
		...
		clinicalEvidence: isEnglish ? 'Clinical Evidence' : 'Клінічні докази',
		veterinaryProtocols: isEnglish ? 'Veterinary Protocols' : 'Ветеринарні протоколи',
		safetyData: isEnglish ? 'Safety Data' : 'Дані безпеки',
		professionalGuide: isEnglish ? 'Professional Guide' : 'Професійний посібник'
	}));
</script>
	...
	<div class="veterinary-cbd-container">
		<header class="veterinary-cbd-hero">
			<div class="veterinary-cbd-badge">
				<span class="badge-text">{isEnglish ? 'Veterinary Professional' : 'Для ветеринарів'}</span>
			</div>
			...
			<div class="veterinary-cbd-meta">
				<span class="veterinary-cbd-author">{isEnglish ? 'By' : 'Автор'}: {data.author}</span>
				<span class="veterinary-cbd-date">{new Date(data.date).toLocaleDateString(lang === 'en' ? 'en-US' : 'uk-UA')}</span>
		...
		<div class="veterinary-cbd-content-grid">
			<aside class="veterinary-cbd-sidebar">
				...
				<div class="veterinary-cbd-key-insights">
					...
					<div class="insights-list">
						...
						<div class="insight-item">
							...
							<div class="insight-content">
								<h4>{isEnglish ? 'Veterinary Focus' : 'Ветеринарний фокус'}</h4>
								<p>{isEnglish ? 'Professional protocols & guidelines' : 'Професійні протоколи та рекомендації'}</p>
							</div>
						</div>
						<div class="insight-item">
							<div class="insight-icon">🔬</div>
							<div class="insight-content">
								<h4>{isEnglish ? 'Clinical Data' : 'Клінічні дані'}</h4>
								<p>{isEnglish ? 'Safety & efficacy evidence' : 'Докази безпеки та ефективності'}</p>
							</div>
						</div>
						<div class="insight-item">
							<div class="insight-icon">📋</div>
							<div class="insight-content">
								<h4>{isEnglish ? 'Treatment Protocols' : 'Протоколи лікування'}</h4>
								<p>{isEnglish ? 'Evidence-based dosing guidelines' : 'Науково обґрунтовані рекомендації дозування'}</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="veterinary-cbd-study-highlights">
					<h3>{isEnglish ? 'Study Highlights' : 'Виділені дослідження'}</h3>
					<ul>
						<li><strong>McGrath (2019):</strong> {isEnglish ? '33% seizure reduction' : '33% зменшення нападів'}</li>
						<li><strong>Rozental (2023):</strong> {isEnglish ? '51 dogs crossover study' : '51 собака, перехресне дослідження'}</li>
						<li><strong>New Zealand (2024):</strong> {isEnglish ? 'THC+CBD combination' : 'Комбінація THC+CBD'}</li>
						<li><strong>Kulpa (2021):</strong> {isEnglish ? 'Safety up to 30 mg/kg' : 'Безпека до 30 мг/кг'}</li>
					</ul>
				</div>
			</aside>
		...
		<section class="veterinary-cbd-related" id="related-articles">
			...
			<div class="veterinary-cbd-related-grid">
				<a href={`${lang}/cbd/safety`} class="veterinary-cbd-related-card">
				...
				<a href={`${lang}/cbd/dogs`} class="veterinary-cbd-related-card">
				...
				<a href={`${lang}/cbd/cats`} class="veterinary-cbd-related-card">
				...
				<a href={`${lang}/pets/thc-toxicity`} class="veterinary-cbd-related-card">
				...
				<a href={`${lang}/cbd`} class="veterinary-cbd-related-card">
				...
				<a href={`${lang}/dog-health`} class="veterinary-cbd-related-card">
					<h3>{isEnglish ? 'Dog Health: Comprehensive Guide' : 'Здоров\'я собак: повний посібник'}</h3>
					<p>{isEnglish ? 'Complete natural health guide for dogs' : 'Повний посібник натурального здоров\'я собак'}</p>
