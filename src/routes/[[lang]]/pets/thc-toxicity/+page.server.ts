...
export const load: PageServerLoad = async ({ params }) => {
	...
	// Путь к MD файлу для статьи о THC токсичности
	...
	if (existsSync(contentPath)) {
		try {
			...
			console.error('Error loading THC toxicity content:', error);
		}
	}
	...
	const defaultDescription =
		currentLocale === 'en'
			? 'Complete guide to THC toxicity in dogs and cats: symptoms, treatment, prevention and CBD product safety considerations.'
			: 'Повний посібник з токсичності THC для собак та котів: симптоми, лікування, профілактика та міркування безпеки CBD продуктів.';
