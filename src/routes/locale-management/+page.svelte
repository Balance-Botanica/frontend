...
<svelte:head>
	...
	<meta
		name="description"
		content="Manage application locales, languages, and internationalization settings"
	/>
</svelte:head>
		...
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			<div class="flex items-center justify-between py-6">
				<div>
					...
					<p class="mt-1 text-gray-600">Manage application locales and internationalization</p>
				</div>
				...
				<div class="flex items-center space-x-4">
					{#if isUserAuthenticated}
						<div class="flex items-center space-x-3">
							...
							<Button on:click={handleSignOut} variant="outline" size="sm">Sign Out</Button>
						</div>
					{:else}
						<Button on:click={() => (showAuth = true)} variant="primary">Sign In</Button>
					{/if}
				</div>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
			...
			<div class="lg:col-span-1">
				<div class="rounded-lg bg-white p-6 shadow">
					<h2 class="mb-4 text-xl font-semibold text-gray-900">Current Locale</h2>
						...
						<div class="space-y-2">
							<div class="flex justify-between text-sm">
								...
								<span class="font-medium"
									>{SUPPORTED_LOCALES[currentLocaleCode]?.dateFormat || 'N/A'}</span
								>
							</div>
							<div class="flex justify-between text-sm">
								<span class="text-gray-600">Time Format:</span>
								<span class="font-medium"
									>{SUPPORTED_LOCALES[currentLocaleCode]?.timeFormat || 'N/A'}</span
								>
							</div>
							<div class="flex justify-between text-sm">
				...
				<div class="mt-6 rounded-lg bg-white p-6 shadow">
					...
					<div class="space-y-3">
						<Button on:click={() => advancedLocaleStore.toggle()} variant="outline" class="w-full">
							Toggle Locale
						</Button>

						<Button on:click={() => advancedLocaleStore.reset()} variant="outline" class="w-full">
							Reset to Default
						</Button>
					</div>
				</div>
			...
			<div class="lg:col-span-2">
				...
				<div class="mb-6 rounded-lg bg-white p-6 shadow">
					<h2 class="mb-4 text-xl font-semibold text-gray-900">Locale Selection</h2>

					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						{#each getEnabledLocales() as locale}
							<button
								on:click={() => handleLocaleChange(locale.code)}
								class="rounded-lg border p-4 text-left transition-all hover:shadow-md {locale.code ===
								currentLocaleCode
									? 'border-main bg-main/5'
									: 'border-gray-200 hover:border-main/30'}"
							>
								<div class="flex items-center justify-between">
									...
									{#if locale.code === currentLocaleCode}
										<div class="h-3 w-3 rounded-full bg-main"></div>
									{/if}
								</div>
								<div class="mt-2 text-xs text-gray-500">
									Priority: {locale.priority} | {locale.currency} | {locale.fallback ||
										'No fallback'}
								</div>
							</button>
						{/each}
				...
				<div class="mb-6 rounded-lg bg-white p-6 shadow">
					<div class="mb-4 flex items-center justify-between">
						...
						<Switch bind:checked={showAdvancedSettings} label="Show Advanced Options" />
					</div>
					...
					{#if showAdvancedSettings}
						<div class="space-y-4">
							<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
								<div class="space-y-3">
									<Checkbox bind:checked={performanceMetrics} label="Performance Metrics" />
									<p class="text-xs text-gray-500">Track locale loading times and bundle sizes</p>
								</div>

								<div class="space-y-3">
									<Checkbox bind:checked={accessibilityFeatures} label="Accessibility Features" />
									<p class="text-xs text-gray-500">Enable screen reader and keyboard navigation</p>
								</div>

								<div class="space-y-3">
									<Checkbox bind:checked={seoOptimization} label="SEO Optimization" />
									<p class="text-xs text-gray-500">Add hreflang and alternate URLs</p>
								</div>
							</div>

							<div class="border-t pt-4">
								<h4 class="mb-3 font-medium text-gray-900">Locale Priority</h4>
	...
	<div class="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black p-4">
		<div class="w-full max-w-md rounded-lg bg-white p-6">
			<div class="mb-4 flex items-center justify-between">
				...
				<button on:click={() => (showAuth = false)} class="text-gray-400 hover:text-gray-600">
			...
			<PhoneAuth on:success={handleAuthSuccess} on:error={handleAuthError} />
		</div>
	</div>
{/if}
